...
for h in az-es-mgs az-es-mds0 az-es-mds1 az-es-oss0 az-es-oss1 az-es-oss2 az-es-oss3; do
	sudo -u $ADMIN_USER sh -c "sshpass -p $ADMIN_PASS ssh-copy-id ${h}"
done

sudo -u $ADMIN_USER clush -w az-es-mgs,az-es-mds[0-1],az-es-oss[0-s31] "sshpass -p $ADMIN_PASS sudo sh -c \"echo $ADMIN_PASS | passwd --stdin root\""
